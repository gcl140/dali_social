...
# from posts.views import general_context
...
def extract_clean_name_from_email(email: str) -> str:
    local = email.split('@')[0]             # gift.c.christian.29

    parts = local.split('.')                # ['gift', 'c', 'christian', '29']

    cleaned = [
        re.sub(r'\d+', '', p).strip()       # remove digits
        for p in parts
        if p and not p.isnumeric()          # skip pure number blocks
    ]

    cleaned = [c for c in cleaned if c]     # remove empty leftovers

    # Capitalize each part
    return " ".join(c.capitalize() for c in cleaned)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not email.endswith('@dartmouth.edu'):
                messages.error(request, "Please use your official Dartmouth email address to register.")
                return redirect('register')

            user = form.save(commit=False)
            user.name = extract_clean_name_from_email(email)
...
def login(request):
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            ...
            if not Member.objects.filter(name=request.user.name).exists():
                member, created = Member.objects.get_or_create(name=request.user.name)
                messages.info(request, "Please complete your profile information.")
                return redirect('member_edit', member_id=member.id)


            if user.is_staff:
                return redirect('post_list')
            else:
                return redirect('post_list')  # Standard redirect — adjust to your default user landing page
