...
const scrollContainerr = document.getElementById("scrollContainer");
const vh = window.innerHeight;
let currentIndexx = 0;
// let isScrolling = false;

document.getElementById("scrollUp").addEventListener("click", () => {
  const totalItems = scrollContainerr.children.length;
  if (isScrolling || totalItems === 0) return;

  if (currentIndexx > 0) currentIndexx--;

  isScrolling = true;
  scrollContainerr.scrollTo({
    top: currentIndexx * vh,
    behavior: "smooth",
  });
...
document.getElementById("scrollDown").addEventListener("click", () => {
  ...
  if (currentIndexx < totalItems - 1) currentIndexx++;
